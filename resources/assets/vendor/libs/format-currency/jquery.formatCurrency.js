...
(function ($) {
    ...
    $.fn.formatCurrencyLive = function (settings) {
        return this.each(function () {
            ...
            if (settings.roundToDecimalPlace === -2)
                ...
                else settings.decPointCharCodes = [46, 190, 110];
            }
    ...
    $.fn.formatCurrency = function (destination, settings) {
        ...
        return this.each(function () {
            ...
            // set destination
            $destination[
                $destination.is("input, select, textarea") ? "val" : "html"
            ](money[0]);
    ...
    $.getFormattedCurrency = function (expr, settings, returnMetadata) {
        ...
        expr =
            typeof expr !== "string"
                ? expr.toString().replace(".", settings.decimalSymbol)
                : expr;

        //identify '(123)' as a negative number
        if (expr.search("\\(") >= 0) expr = "-" + expr;

        if (
            expr === "" ||
            (expr === "-" && settings.roundToDecimalPlace === -1)
        )
        ...
        var hasDecimals =
            numParts.length > 1 && settings.roundToDecimalPlace > -2;
        ...
        if (settings.groupDigits) {
            for (var i = 0; i < Math.floor((expr.length - (1 + i)) / 3); i++) {
                expr =
                    expr.substring(0, expr.length - (4 * i + 3)) +
        ...
        if (
            ...
            if (settings.removeTrailingZerosOnDecimal) {
                ...
                if (decimals.length < settings.minimumDecimalPlaces)
                    decimals = String(
                        decimals +
                            Array(settings.minimumDecimalPlaces + 1).join("0")
                    ).slice(0, settings.minimumDecimalPlaces);
            }
            expr +=
                decimals.length > 0 ? settings.decimalSymbol + decimals : "";
        }
        ...
        return returnMetadata
            ? [money, hasDecimals, originalDecimals, isPositive, settings]
            : money;
    };
    ...
    function buildSettingsObjGraph(settings, defaults) {
        ...
        if (settings.region !== "")
            $.extend(
                settings,
                getRegionOrCulture(settings.region),
                bareSettings
            );

        if (settings.suppressCurrencySymbol) {
            settings.symbol = "";
            settings.positiveFormat = $.trim(
                settings.positiveFormat.replace("%s", "")
            );
            settings.negativeFormat = $.trim(
                settings.negativeFormat.replace("%s", "").replace(" %n", "%n")
            );
        }

        if (settings.roundToDecimalPlace >= -1) {
            settings.minimumDecimalPlaces =
                settings.roundToDecimalPlace == -1 ||
                settings.minimumDecimalPlaces > settings.roundToDecimalPlace
                    ? settings.roundToDecimalPlace
                    : settings.minimumDecimalPlaces;
        } else {
        ...
        if (settings.symbol === "")
            settings.regex = new RegExp(
                "[^\\d" + settings.decimalSymbol + "-]",
                "g"
            );
        else {
            var symbol = settings.symbol
                ...
                .replace(".", "\\.");
            settings.regex = new RegExp(
                symbol + "|[^\\d" + settings.decimalSymbol + "-]",
                "g"
            );
        }

        settings.regexGroupDigit = new RegExp(
            "\\" + settings.digitGroupSymbol,
            "g"
        );
    ...
    function keyAllowed(ev, decPointCodes) {
        if (ev.which >= 48 && ev.which <= 57) return true;
        else if (ev.type == "keypress" && ev.which == decPointCodes[0])
            return (
                ev.target.value.indexOf(
                    String.fromCharCode(decPointCodes[0])
                ) == -1
            );
